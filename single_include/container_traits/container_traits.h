...
template <typename C, typename Ret, typename... Args>
struct has_begin<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().begin(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_cbegin<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().cbegin(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_end<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().end(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_cend<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().cend(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_rbegin<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().rbegin(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_crbegin<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().crbegin(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_rend<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().rend(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_crend<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().crend(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().data(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_front<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().front(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_back<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().back(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_max_size<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().max_size(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().capacity(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_shrink_to_fit<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().shrink_to_fit(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_insert<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().insert(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_insert_or_assign<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().insert_or_assign(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_emplace_hint<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().emplace_hint(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_try_emplace<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().try_emplace(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_erase<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().erase(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_push_front<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().push_front(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_emplace_front<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
...
template <typename C, typename Ret, typename... Args>
struct has_pop_front<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().pop_front(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_pop_back<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().pop_back(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_merge<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().merge(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_extract<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().extract(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_splice<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().splice(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_remove<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().remove(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_remove_if<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().remove_if(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_reverse<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().reverse(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_unqiue<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().unqiue(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_sort<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().sort(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_contains<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().contains(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_lower_bound<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().lower_bound(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_upper_bound<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().upper_bound(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_equal_range<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().equal_range(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_key_comp<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().key_comp(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_value_comp<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().value_comp(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_hash_function<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().hash_function(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
template <typename C, typename Ret, typename... Args>
struct has_key_eq<C, Ret(Args...)> {
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().key_eq(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
...
private:
  template <typename T>
  static constexpr auto check(T *) -> typename std::is_same<
      decltype(std::declval<T>().get_allocator(std::declval<Args>()...)),
      Ret
      >::type; // attempt to call it and see if the return type is correct
